// Test script to verify real employee creation and login (v2)
// Run this with: node test-real-employee-creation-v2.js
...
async function testRealEmployeeCreationV2() {
  try {
    console.log('Testing real employee creation and login (v2)...');
    
    // Generate unique timestamps for emails
    const timestamp = Date.now();
    
    // First, create a real owner account
    const ownerEmail = `real-owner-${timestamp}@example.com`;
    const ownerPassword = 'real123456';
    const ownerName = 'Real Owner V2';
    const restaurantId = `real_restaurant_${timestamp}`;
    ...
    const employeeEmail = `real-employee-${timestamp}@example.com`;
    const employeePassword = 'real123456';
    const employeeName = 'Real Employee V2';
    ...
    // Create restaurant user mapping for employee (in restaurant's users collection)
    await setDoc(doc(firestore, `restaurants/${restaurantId}/users`, employeeUser.uid), {
      id: employeeUser.uid,
      email: employeeEmail,
      restaurantId: restaurantId,
      role: 'Staff',
