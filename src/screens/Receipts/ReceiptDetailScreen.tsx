...
Generated by ARBI POS System
    ...
    const splitText = Array.isArray((order.payment as any)?.splitPayments) && (order.payment as any).splitPayments.length > 0
      ? `Split Breakdown:\n${(order.payment as any).splitPayments.map((sp: any) => ` - ${sp.method}: Rs. ${Number(sp.amount || 0).toFixed(2)}`).join('\n')}`
      : '';
    const name = (restaurantInfo?.name || authRestaurantName || 'ARBI POS').toString();
    ...
    const processedBy = order?.processedBy?.role && order?.processedBy?.username 
      ? `${order.processedBy.role}:${order.processedBy.username}`
            ...
            <>
              ...
              <View style={styles.headerRow}>
                <View style={styles.restaurantHeader}>
                  ...
                  <Text style={styles.restaurantName}>{(restaurantInfo?.name || authRestaurantName || 'ARBI POS').toString()}</Text>
                  {!!restaurantInfo?.address && (
