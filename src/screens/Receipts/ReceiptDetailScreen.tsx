...
Thank you!
Powered by House of Job Pvt. Ltd
      `.trim();
    ...
    const splitText = Array.isArray((order.payment as any)?.splitPayments) && (order.payment as any).splitPayments.length > 0
      ? `\nSplit Payment Breakdown:\n${(order.payment as any).splitPayments.map((sp: any) => `  ${sp.method}: Rs. ${Number(sp.amount || 0).toFixed(2)}`).join('\n')}\n  Total Split: Rs. ${((order.payment as any).splitPayments as any[]).reduce((sum: number, sp: any) => sum + Number(sp.amount || 0), 0).toFixed(2)}`
      : '';
    ...
    const paymentBlock = order.payment ? (() => {
      const totalSplitAmount = Array.isArray((order.payment as any)?.splitPayments) && (order.payment as any).splitPayments.length > 0
        ? ((order.payment as any).splitPayments as any[]).reduce((sum: number, sp: any) => sum + Number(sp.amount || 0), 0)
        : order.payment.amountPaid;
      
      const change = order.payment.method === 'Split' 
        ? Math.max(0, totalSplitAmount - total) // For split payments, change is total split minus order total
        : Math.max(0, order.payment.amountPaid - total); // For single payments, change is amount paid minus order total
      
      return `\nPayment:\nMethod: ${order.payment.method}\nAmount Paid: Rs. ${order.payment.amountPaid.toFixed(1)}\nChange: Rs. ${change.toFixed(1)}\n${splitText}`;
    })() : '';
        ...
        <View style={styles.receiptPaper}>
          {isCreditSettlement ? (
            ...
            <>
              ...
              <View style={styles.creditSection}>
                ...
                <View style={styles.headerRow}>
                  <View style={styles.restaurantHeader}>
                    <Text style={styles.restaurantName}>House of Job Pvt. Ltd</Text>
              ...
              <View style={styles.creditSection}>
                ...
                <View style={styles.footer}>
                  ...
                  <Text style={styles.footerText}>Generated by House of Job Pvt. Ltd System</Text>
                </View>
              </View>
            </>
          ) : (
            ...
            <>
              ...
              {order.payment && (
                <>
                  <View style={styles.paymentSection}>
                    ...
                    {(order.payment.method === 'Split' && Array.isArray((order.payment as any)?.splitPayments) && (order.payment as any).splitPayments.length > 0) && (
                      <View style={{ marginTop: 8 }}>
                        <Text style={[styles.paymentLabel, { textAlign: 'center', fontWeight: 'bold' }]}>Split Payment Breakdown</Text>
                        {((order.payment as any).splitPayments as any[]).map((sp: any, idx: number) => (
                        ...
                        <View style={[styles.paymentRow, { borderTopWidth: 1, borderTopColor: '#ddd', paddingTop: 4, marginTop: 4 }]}>
                          <Text style={[styles.paymentLabel, { fontWeight: 'bold' }]}>Total Split:</Text>
                          <Text style={[styles.paymentValue, { fontWeight: 'bold' }]}>
                            Rs {((order.payment as any).splitPayments as any[]).reduce((sum: number, sp: any) => sum + Number(sp.amount || 0), 0).toFixed(2)}
                          </Text>
                        </View>
                      </View>
                    )}
              ...
              <View style={styles.footer}>
                <Text style={styles.footerText}>Thank you</Text>
                <Text style={styles.footerText}>Powered by House of Job Pvt. Ltd</Text>
